...
export default {
	...
	onLoad({ id }) {
		...
		this.order = Member.orders.find(item => item.order_no == id);
	...
	methods: {
		review() {
			...
			uni.navigateTo({
				url: '/pages/review/review?storename=' + this.order.store.name + '&postscript=' + this.order.postscript + '&date=' + date + '&order_no=' + this.order.order_no
			})
		},
		goToInvoice() {
			uni.navigateTo({
