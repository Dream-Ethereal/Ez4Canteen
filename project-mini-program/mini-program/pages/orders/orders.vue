<template>
	<view class="container">
		<view class="orders-list d-flex flex-column w-100" style="padding: 20rpx; padding-bottom: 0;">
			<view class="order-item" v-for="(item, index) in orders" :key="index" style="margin-bottom: 30rpx;" @tap="detail(item.order_no)">
					...
					<view class="w-100 d-flex flex-column">
						...
						<view class="d-flex align-items-center justify-content-end">
							<view style="margin-right: 10rpx;">
								<button type="primary" plain size="mini">查看发票</button>
	...
	export default {
		...
		methods: {
			...
			review(order) {
				...
				uni.navigateTo({
					url: '/pages/review/review?storename=' + order.store.name + '&postscript=' + order.postscript + '&date=' + date + '&order_no=' + order.order_no
				})
			},
			goToInvoice() {
				uni.navigateTo({
